...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned int x = 0; x < image.width(); x++) {
    ...
    HSLAPixel & pixel = image.getPixel(x, y);
    if (distance(centerX, centerY, x, y) > 160) {
      pixel.l = pixel.l * 0.2;
    } else
      pixel.l = pixel.l * (1 - (0.005 * distance(centerX, centerY, x, y)));
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for (unsigned int x = 0; x < firstImage.width(); x++) {
    ...
    HSLAPixel & spotlight_pixel = secondImage.getPixel(x, y);
    if (spotlight_pixel.l == 1.0 && pixel.l <= 0.8) {
      pixel.l+= 0.2;
  }
}
}
