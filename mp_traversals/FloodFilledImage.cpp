...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
	Animation animation;
	/** @todo [Part 2] */
	auto traverse = traversal_.begin();
	auto color = color_.begin();
	while (traverse != traversal_.end() && color != color_.end()) {
		ImageTraversal* currTraversal = *traverse;
		ColorPicker* currColor = *color;
		unsigned frameNum = 0;
	for (ImageTraversal::Iterator it = currTraversal->begin(); it.getEnd() != true; ++it) {
		if (frameNum % frameInterval == 0) {
			animation.addFrame(*png_);
		}
		HSLAPixel & original = png_->getPixel((*it).x, (*it).y);
		original = currColor->getColor((*it).x, (*it).y);
		frameNum++;
	}
	animation.addFrame(*png_);
	++traverse;
