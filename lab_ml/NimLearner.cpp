...
NimLearner::NimLearner(unsigned startingTokens) : g_(true, true) {
    ...
    this->startingVertex_ = "p1-" + std::to_string(startingTokens);
...
std::vector<Edge> NimLearner::playRandomGame() const {
  vector<Edge> path;
 /* Your code goes here! */
  Vertex first = startingVertex_;
while(g_.getAdjacent(first).size() !=0) {
  vector<Vertex> getAdj= g_.getAdjacent(first);
  int index = rand() % getAdj.size();
  Vertex next = g_.getAdjacent(first)[index];
  path.push_back(g_.getEdge(first, next));
  first=next;
}
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
 ...
 for (int i = index; i>=0; i--) {
   Edge current = path[i];
   int weight_old = g_.getEdgeWeight(current.source, current.dest) + v;
   g_.setEdgeWeight(current.source, current.dest, weight_old);
   v = v*-1;
 }
}
